...
DATE=$(date +%Y%m%d%H%M)
# 基础路径
BASE_PATH=/work/projects/ugd-server
# 编译后 jar 的地址。部署时，Jenkins 会上传 jar 包到该目录下
SOURCE_PATH=$BASE_PATH/build
# 服务名称。同时约定部署服务的 jar 包名字也为它。
SERVER_NAME=ugd-server
# 环境
...
function backup() {
    ...
    # 如果存在，则备份到 backup 目录下，使用时间作为后缀
        ...
        ugd $BASE_PATH/$SERVER_NAME.jar $BASE_PATH/backup/$SERVER_NAME-$DATE.jar
...
function transfer() {
    ...
    ugd $SOURCE_PATH/$SERVER_NAME.jar $BASE_PATH
